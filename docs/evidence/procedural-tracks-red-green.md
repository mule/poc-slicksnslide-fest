...
**Update:** the transcripts above predate the open-surface change and describe a test structure that no longer exists. The per-segment track fence they exercised — the boundary `SegmentShape2D` collision, the straight/curved sample and joint sweeps, and the 28-contacting-joint traversal (along with its `SKIP_JOINT_TRAVERSAL_SWEEP` skip) — was retired in this branch. `TrackRuntime` now builds no per-segment walls at all; the only collision geometry is a single four-segment `StaticBody2D` far outside the track (see [The open surface](open-surface.md)). Issue #21, which tracked the stalled joint-traversal sweep, is closed. `tests/track_collision_physics_test.gd` now verifies containment directly: it drives a probe outward from the centerline toward each side of the play area and checks both that it is stopped inside a tolerance-grown play-area rectangle and that it actually reached that boundary, plus a `--break-collision` mode that strips the containment shapes and expects the probe to escape instead.
