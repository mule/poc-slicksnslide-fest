...
The desktop prototype budget is 50 ms for one circuit and 1 second for the complete seeds `0..9` headless verification. A recorded Godot 4.7.1 run generated individual seeds in roughly 11–23 ms and completed the suite, including a second deterministic generation of the first 10 seeds, in approximately 420 ms; those per-run timings are machine-dependent. The generator test now sweeps seeds `0..19` and observably completes **745 checks** (verified against the current `tests/track_generator_test.gd` run). Mobile timing remains intentionally deferred to the platform-specific issues.
