...
Automatic reset and safe-pose capture are both ground-only: neither runs while the car is airborne, and no safe pose is recorded on a ramp or its flank, so a jump over open ground is never mistaken for a car that is stuck or lost and a reset never puts the car back onto a ramp face. The gate reads the height sample's `on_feature` flag rather than the height itself, because the surface is no longer flat: since epic #46 the whole play area is a terrain field (±52.5 px, see [The terrain field](terrain.md)) with the ramps summed onto it, so "raised" no longer means "on a ramp" — on seed 13 the entire road sits above zero. Off-track ground has shape too, and a car reset onto a slope sits still there (`low_speed_stabilization` exceeds gravity on the steepest slope the catalog allows). The height channel was deferred when this document was written; it is now implemented as sub-project C, and [The height channel](height-channel.md) is the contract for it.
