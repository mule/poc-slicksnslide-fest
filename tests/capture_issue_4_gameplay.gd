...
func _capture() -> void:
	...
	_save_svg("%s/frame_%03d.svg" % [FRAME_DIRECTORY, _frame_index - 1], 479)
	_last_video_frame_collision_count = _car.get_collision_count()
...
func _drive_tick(tick: int) -> void:
	if tick < 150:
		...
		if tick == 300:
			_car.set_safe_reset_pose(Transform2D(0.0, Vector2(320.0, 430.0)))
...
func _save_svg(path: String, tick: int) -> void:
	...
	var lines := PackedStringArray([
		...
		"<line x1=\"%.1f\" y1=\"%.1f\" x2=\"%.1f\" y2=\"%.1f\" stroke=\"#68d4ff\" stroke-width=\"3\" stroke-dasharray=\"8 6\"/>" % [_car.global_position.x, _car.global_position.y, lead_target.x, lead_target.y],
		"<circle cx=\"%.1f\" cy=\"%.1f\" r=\"7\" fill=\"#68d4ff\"/>" % [lead_target.x, lead_target.y],
		"<g transform=\"translate(%.1f %.1f) rotate(%.1f)\">" % [_car.global_position.x, _car.global_position.y, rad_to_deg(_car.global_rotation)],
		...
		"<text x=\"38\" y=\"136\" fill=\"#ffdd72\" font-family=\"monospace\" font-size=\"17\">contacts %d   %s   center y %.1f</text>" % [_car.get_collision_count(), "POST-IMPACT" if _car.get_collision_count() > 0 else "APPROACH", _car.global_position.y],
...
func _add_wall(arena: Node2D) -> void:
	...
	wall.position = Vector2(320.0, 350.0)
	var shape_node := CollisionShape2D.new()
	var shape := RectangleShape2D.new()
	shape.size = Vector2(580.0, 22.0)
